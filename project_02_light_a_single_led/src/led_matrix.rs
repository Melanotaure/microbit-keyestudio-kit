...
pub async fn plot_xy(x: u8, y: u8) {
    // Lights up a single LED on the matrix
    let mut mat = EMPTY_CHAR;

    mat[y as usize][x as usize] = 1;

    DISPLAY_SIGNAL.signal(mat);
}

pub async fn unplot_xy(x: u8, y: u8) {
    // Lights up a single LED on the matrix
    let mut mat = EMPTY_CHAR;

    mat[y as usize][x as usize] = 0;
